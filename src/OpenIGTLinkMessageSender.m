...
function result = WriteOpenIGTLinkStringMessage(deviceName, msgString, protocolVersion)
    ...
    msg.dataTypeName = padString('STRING', 12);
    msg.deviceName = padString(deviceName, 20);
    % STRING content
    msgString = [uint8(msgString) uint8(0)];
...
function result = WriteOpenIGTLinkTransformMessage(deviceName, transform, protocolVersion)
    ...
    msg.dataTypeName = padString('TRANSFORM', 12);
    msg.deviceName = padString(deviceName, 20);
    % TRANSFORM content
    msg.content = [];
...
function result = WriteOpenIGTLinkPointMessage(deviceName, pointList, protocolVersion)
    ...
    msg.dataTypeName = padString('POINT', 12);
    msg.deviceName = padString(deviceName, 20);
    % POINT content
    msg.content = [];
    ...
    for i = 1:numPoints
        name = padString([deviceName, '-', num2str(i)],64);
        group = padString('Reserved', 32);
